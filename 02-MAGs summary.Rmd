...
#Add KEGG heatmap
kegg_tree <- gheatmap(kegg_tree, genome_kegg, offset=0.5, width=3.5, colnames=FALSE) +
  vexpand(.08) +
...
tSNE_func <- Rtsne(X=genome_kegg, dims = 2, check_duplicates = FALSE)

# Plot the ordination
tSNE_func$Y %>%
  as.data.frame() %>%
  mutate(genome=rownames(genome_kegg)) %>%
...
#Add counts
vertical_tree <- gheatmap(vertical_tree, mutate_if(genome_counts, is.numeric,log10), offset=0.04, width=3.5, colnames=TRUE, colnames_angle=90, font.size=2, colnames_position="top", colnames_offset_y = 9) +
  vexpand(.08) +
  coord_cartesian(clip = "off") +
...
read_counts_cov_size_pivot <- read_counts_cov_size %>%
...
read_counts_cov_size_by_host <- sample_metadata %>%
  select("sample","species","Area_type") %>%
  rename(host_sp=species) %>%
  left_join(read_counts_cov_size_pivot,., by=join_by("sample" == "sample")) #%>%
...
phylum_colors <- ehi_phylum_colors %>%
  filter(phylum %in% unique(read_counts_cov_size_by_host$phylum)) %>%
  select(colors) %>%
  pull() %>%
  rev()
...
ggplot(read_counts_cov_size_by_host, aes(x=sample,y=count,fill=phylum, group=phylum))+ #grouping enables keeping the same sorting of taxonomic units
