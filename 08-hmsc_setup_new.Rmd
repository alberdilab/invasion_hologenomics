...
red_samples <- sample_metadata %>%
  ...
  filter(species == "Sciurus vulgaris",
         development=="Adult",  #remove juveniles, nursing and pregnant females
         est_mapp>0.8) %>%  #remove samples with low DAMR
  dplyr::select(sample) %>%
  pull()
...
selected_genomes_red0 <- genome_counts %>%
    column_to_rownames(var="genome") %>%
    select(any_of(red_samples)) %>%
    filter(rowSums(across(starts_with("EHI")) != 0) >= 5) %>%
    rownames()

selected_genomes_grey0 <- genome_counts %>%
    column_to_rownames(var="genome") %>%
...
selected_genomes_red <- genome_counts %>%
    filter(genome %in% selected_genomes_red0) %>%
    column_to_rownames(var="genome") %>%
    hilldiv2::tss() %>%
    as.data.frame() %>%
...
selected_genomes_grey <- genome_counts %>%
    filter(genome %in% selected_genomes_grey0) %>%
    column_to_rownames(var="genome") %>%
    hilldiv2::tss() %>%
    as.data.frame() %>%
    filter(rowSums(across(starts_with("EHI")) >= 0.01) >= 5) %>%
...
# genome_metadata_red <- genome_metadata %>%
#     filter(genome %in% selected_genomes_red)
# 
# genome_metadata_grey <- genome_metadata %>%
#     filter(genome %in% selected_genomes_grey)
...
YData.red <- read_counts  %>%
                    filter(genome %in% selected_genomes_red) %>% #subset genomes
...
XData.red <- sample_metadata %>%
                    filter(species=="Sciurus vulgaris") %>%
                    select(sample,sex,index500,season) %>%
...
YData.grey <- read_counts  %>%
                    filter(genome %in% selected_genomes_grey) %>% #subset genomes
                    mutate(across(where(is.numeric), ~ round(. * normalisation_factor,0) )) %>% 
                    mutate(across(where(is.numeric), ~ . +1 )) %>% #add +1 pseudocount to remove zeros
                    mutate(across(where(is.numeric), ~  log(.) )) %>% #log-transform
                    column_to_rownames("genome") %>%
...
XFormula = ~sex + index500 + season + logseqdepth
