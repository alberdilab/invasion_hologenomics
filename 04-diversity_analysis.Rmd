...
# neutral.fit <-diversity.data %>%
#     group_by(species) %>%
#     add_predicted_draws(neutral.brm) %>%  # adding the posterior distribution
#     ggplot(aes(x = index500, y = neutral, color = ordered(species), fill = ordered(species))) +  
#     stat_lineribbon(aes(y = .prediction), .width = c(.95),  # regression line and CI
#                     alpha = 0.5, colour = "black") +
#     geom_point(data = diversity.data, colour = "darkseagreen4", size = 3) +   # raw data
#     scale_fill_brewer(palette = "Greys") +
#     ylab("neutral Hill numbers") +  # latin name for red knot
#     xlab("index500") +
#     theme_bw() +
...
diversity.data %>%
  data_grid(index500 = seq_range(index500, n = 100), .model=neutral.brm) %>%
  add_predicted_draws(neutral.brm) %>%
  ggplot(aes(x = index500, y = neutral)) +
  stat_lineribbon(aes(y = .prediction), .width = c(.99, .95, .8, .5), color = "#08519C") +
  geom_point(data = diversity.data, size = 2) +
  scale_fill_brewer()

conditional_effects(neutral.brm)
...
phylo.brm <-brm(neutral ~ species + index500 + season + 
            + species:index500 + species:season 
            + (1|animal) + (1|sampling_site),
            data=diversity.data)
...
# neutral.fit <-diversity.data %>%
#     group_by(species) %>%
#     add_predicted_draws(neutral.mbrm) %>%  # adding the posterior distribution
#     ggplot(aes(x = index500, y = neutral, color = ordered(species), fill = ordered(species))) +  
#     stat_lineribbon(aes(y = .prediction), .width = c(.95),  # regression line and CI
#                     alpha = 0.5, colour = "black") +
#     geom_point(data = diversity.data, colour = "darkseagreen4", size = 3) +   # raw data
