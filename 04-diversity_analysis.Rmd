...
hr <- with(sample_metadata_adonis, how(nperm = 999, blocks = animal)) #accounting for repeated measures?

adonis2(formula=beta_q1n ~ species + index500 + season + sex + development, 
        data=sample_metadata_adonis[labels(beta_q1n),], permutations=hr) %>%
  as.matrix() %>%
  print()
...
sample_metadata_adonis <- sample_metadata %>%
  ...
  arrange(sample) %>%
  #mutate(location=paste0(round(longitude,2),"_",round(latitude,2))) %>%
  select(sample,animal,species,index500,season) %>%
  select_if(~ length(unique(.)) > 1) %>% #remove columns with all-identical values
  column_to_rownames(var = "sample") %>%
  as.data.frame()
...
library("permute")
hr <- with(sample_metadata_adonis, how(nperm = 999, blocks = animal)) #accounting for repeated measures
h1 <- how(plots = Plots(strata = sample_metadata_adonis$species, type = "none"),
         nperm = 999)
h2 <- with(sample_metadata_adonis, 
           how(nperm = 999, 
               blocks = species,
               plots = Plots(strata = animal, type = "free"),
               )) #doesn't really work on my data because of unbalanced design

adonis2(formula=beta_q1n ~ species + season + index500 ,
        method="bray",data=sample_metadata_adonis[labels(beta_q1n),], permutations=hr) %>% #by="margin" for marginal effects
  as.matrix() %>%
  print()

dbRDA=capscale(beta_q1n ~ species + index500 + season, sample_metadata_adonis[labels(beta_q1n),],
dist="bray")
plot(dbRDA)
anova(dbRDA)
...
ggplot(data = data.scores, aes(x = NMDS1, y = NMDS2)) + 
     geom_point(data = data.scores, aes(shape = species, colour=season), size = 3, alpha = 0.5) + 
