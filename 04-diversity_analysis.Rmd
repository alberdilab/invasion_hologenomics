...
dispersion <- betadisper(beta_q1n, group=sample_metadata_adonis$species)
#plot(dispersion, main="PCoA")
boxplot(dispersion$distances ~ dispersion$group, main="Distance to centroids")
permutest(dispersion)

# pairwise.adonis(beta_q1n, group=sample_metadata_adonis$species, sim_method = "bray",
#   p_adjust_m = "bonferroni", reduce = NULL)

pairwise.adonis2(beta_q1n ~ season, data = sample_metadata_adonis[labels(beta_q1n),], strata='species', nperm=999)

```



```{r PROVA, eval=TRUE, fig.width=10}
...
beta_q1n_nmds <- beta_q1n %>%
  ...
  left_join(sample_metadata, by = join_by(sample == sample)) %>%
  # mutate(group=ifelse(species=="Sciurus vulgaris" & season=="spring-summer", 'RS',
  #                     ifelse(species=="Sciurus vulgaris" & season=="autumn", 'RA',
  #                            ifelse(species=="Sciurus vulgaris" & season=="winter", 'RW',
  #                                   ifelse(species=="Sciurus carolinensis" & season=="spring-summer", 'GS',
  #                                          ifelse(species=="Sciurus vulgaris" & season=="autumn", 'GA', 'GW')
  #                                          ))))) %>%
  filter(sample !="EHI00420")  #remove outliers

stressplot(beta_q1n_nmds)

env <- beta_q1n_nmds %>% select(species, index500, season)
...
data.scores$season = beta_q1n_nmds$season
data.scores$species = beta_q1n_nmds$species
# data.scores$group = beta_q1n_nmds$group
...
data.scores %>% group_by(species) %>%
  mutate(x_cen = mean(NMDS1, na.rm = TRUE)) %>%
  mutate(y_cen = mean(NMDS2, na.rm = TRUE)) %>%
  ungroup() %>%
  ggplot(., aes(x = NMDS1, y = NMDS2)) + 
  geom_point(size = 3, alpha=0.8, aes(colour = season, shape=species)) + 
  ...
  geom_text(data = vec.df,
            ...
            size=4) +
  #stat_ellipse(data = data.scores, aes(x = NMDS1, y = NMDS2, group = group)) +
  labs(x = "NMDS1", y = "NMDS2", shape = "Species") +
