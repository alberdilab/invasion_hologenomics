...
phyla_legend <- ehi_phylum_colors %>%
  ...
  ggplot() +
  geom_blank() +
  #geom_rect(aes(ymin = 1:(nrow(phyla)) - 0.5, ymax = 1:(nrow(phyla)) + 0.5, xmin = 0, xmax = 0.05, size=0.1, fill = phylum)) +
  geom_tile(aes(x=-1, y=1:nrow(phyla), fill=phylum, width=0.05, xmax=0.06)) +
  scale_fill_manual(values=rev(phyla$colors)) +
  geom_text(aes(y = 1:(nrow(phyla)), x = 0.06, label = rev(phylum)), angle = 0, hjust = 0, size = 3) + 
  theme_void() +
...
#,fig.height=8, fig.width=10, fig.fullwidth=TRUE
...
kegg_tree <- gheatmap(kegg_tree, heatmap, offset=0, width=0.1, colnames=FALSE, color=NA) +
   scale_fill_manual(values=colors_alphabetic)
...
#Add KEGG heatmap
kegg_tree <- gheatmap(kegg_tree, kegg_table, offset=0.5, width=3.5, colnames=FALSE, color=NA) +
  vexpand(.08) +
...
singlem <- sequence_fractions %>%
  ...
  left_join(sample_metadata, by = join_by(sample == sample))  %>%
  mutate(singlem_proportion = round(singlem_fraction*100,2)) %>%
  mutate(est_mapp=singlem_proportion-mags_proportion) %>%
  select(sample,est_mapp)
...
#Add phylum colors
vertical_tree <- gheatmap(vertical_tree, heatmap, offset=0, width=0.1, colnames=FALSE, color=NA) +
  scale_fill_manual(values=colors_alphabetic)
...
```{r taxonomic_composition_by_host, fig.height=7, fig.width=10, fig.fullwidth=TRUE}
...
ggplot(genome_counts_by_host, aes(x=sample,y=count,fill=phylum, group=phylum))+ #grouping enables keeping the same sorting of taxonomic units
  ...
  facet_nested(~host_sp + area_type, scales="free", space="free") +
