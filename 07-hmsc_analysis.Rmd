...
#Phylums
phylum_colors_2 <- read_tsv("https://raw.githubusercontent.com/earthhologenome/EHI_taxonomy_colour/main/ehi_phylum_colors.tsv") %>%
...
varpart_tree <- gheatmap(varpart_tree, phylum_colors_2, offset=-0.2, width=0.1, colnames=FALSE) +
...
#Add phylum colors next to the tree tips
postestimates_tree <- gheatmap(postestimates_tree, phylum_colors_2, offset=-0.2, width=0.1, colnames=FALSE) +
      scale_fill_manual(values=colors_alphabetic)+
      labs(fill="Phylum")
...
grid.arrange(grobs = list(vtreeD,matrix,vtree),
...
pred_species %>%
      ...
      ggplot(., aes(y=genome, x=diff.red_grey, fill=phylum, color=phylum)) +
          ...
          scale_fill_manual(values=alpha(phylum_colors,0.3))+
