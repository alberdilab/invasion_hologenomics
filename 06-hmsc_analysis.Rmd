...
load("hmsc/fit_model8_250_10.Rdata")
...
varpart$vals %>%
   ...
   mutate(variable=factor(variable, levels=c("species","builtup","season","builtup:species","species:season","logseqdepth","Random: animal"))) %>%
   group_by(variable) %>%
...
varpart_table <- varpart$vals %>%
   ...
   rownames_to_column(var="variable") %>%
   pivot_longer(!variable, names_to = "genome", values_to = "value") %>%
   mutate(variable=factor(variable, levels=rev(c("species","builtup","season","builtup:species","species:season","logseqdepth","Random: animal")))) %>%
...
vertical_tree <-  varpart_tree +
        scale_fill_manual(values=c("#34738f","#cccccc","#ed8a45","#b2b530","#be3e2b","#83bb90","#f6de6c"))+
...
post_beta <- getPostEstimate(hM=m, parName="Beta")$support %>%
    ...
    rename(intercept=2,
             ...
             logseqdepth=7,
             builtup_species=8,
             species_season_autumn=9,
             species_season_winter=10) %>%
...
pred_species <- constructGradient(m, 
             ...
             as.data.frame() %>%
             mutate(species=c("Sciurus carolinensis","Sciurus vulgaris")) %>%
...
pred_species %>%
      ...
      mutate(diff = `Sciurus carolinensis` - `Sciurus vulgaris`) %>%
      select(genome, diff) %>%
      left_join(genome_metadata, by=join_by(genome==genome)) %>%
      mutate(genome= factor(genome, levels=genome_tree_subset$tip.label)) %>%
      ggplot(., aes(y=genome, x=diff, fill=phylum, color=phylum)) +
...
# Overall area type prediction
pred_area <- constructGradient(m, 
                      focalVariable = "builtup", 
                      non.focalVariables = 1, 
                      ngrid=gradientlength) %>%
             predict(m, Gradient = ., expected = TRUE) %>%
            do.call(rbind,.) %>%
            as.data.frame() %>%
            mutate(builtup=rep(gradient,1000)) %>%
            pivot_longer(-builtup, names_to = "genome", values_to = "value") %>%
             mutate(genome = sub("(.*\\..*\\.)[^.]+.*", "\\1", genome)) %>% #remove iteration suffix
...
pred_area %>%
      mutate(genome=factor(genome, levels=genome_tree_subset$tip.label)) %>%
      group_by(builtup,genome) %>%
      summarise(mean=mean(value)) %>%
      ggplot(., aes(y=genome, x=builtup, fill=mean, color=mean)) +
          geom_tile() +
...
# Area type Sciurus vulgaris prediction
pred_type_SciVu <- constructGradient(m, 
                      focalVariable = "area_type", 
                      non.focalVariables = list(species=list(3,"Sciurus vulgaris")), 
                      ngrid=gradientlength) %>%
             predict(m, Gradient = ., expected = TRUE)

# Area type  Sciurus carolinensis prediction
pred_type_SciCa <- constructGradient(m, 
                      focalVariable = "area_type", 
