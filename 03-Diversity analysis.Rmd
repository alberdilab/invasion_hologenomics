...
alpha_div <- cbind(sample=colnames(count_table_cov_size),richness=q0n,neutral=round(q1n,3),phylo=round(q1p,3),func=round(q1f,3)) %>% 
  as.data.frame()
columns <- c("richness","neutral","func","phylo", "mapped","total") 
...
alpha_div %>%
  ...
  ggboxplot(., x = "species", y = "value", color = "species", fill="white") +
...
alpha_div %>%
  ...
  mutate(data = factor(data, levels = columns))   %>%
  left_join(sample_table, by = join_by(sample == sample)) %>%
  ggboxplot(., x = "species", y = "value", color = "Area_type", fill="white") +
...
```{r}
#functional alpha by species*area_type
alpha_div %>%
  select(sample,phylo) %>%
  pivot_longer(-sample, names_to = "data", values_to = "value") %>%
  mutate(data = factor(data, levels = columns))   %>%
...
alpha_div %>%
  ...
  mutate(data = factor(data, levels = columns))   %>%
  left_join(sample_table, by = join_by(sample == sample)) %>%
  ggboxplot(., x = "species", y = "value", color = "Season", fill="white") +
...
#functional alpha by species*season
alpha_div %>%
  select(sample,func) %>%
  pivot_longer(-sample, names_to = "data", values_to = "value") %>%
  mutate(data = factor(data, levels = columns))   %>%
...
```{r}
#functional beta diversity NMDS
...
adonis2(formula=beta_q1n_fun$S ~ ., data=sample_table_adonis[labels(beta_q1n_fun$S),], permutations=999) %>%
  as.matrix() %>%
  print()
```
