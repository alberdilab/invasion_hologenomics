...
present_MAGs2 <- present_MAGs[present_MAGs %in% rownames(genome_gifts)]
genome_gifts_filt <- genome_gifts[present_MAGs2,] %>%
...
GIFTs_elements_community <- genome_counts_filt[rownames(GIFTs_elements),] %>%
  tss() %>%
  to.community(GIFTs_elements,.,GIFT_db2)

GIFTs_functions_community <- genome_counts_filt[rownames(GIFTs_functions),] %>%
  tss() %>%
  to.community(GIFTs_functions,.,GIFT_db2)

GIFTs_domains_community <- genome_counts_filt[rownames(GIFTs_domains),] %>%
  tss() %>%
...
# GIFTs_elements[,"S0301"]
# GIFTs_functions[,'S03']
# GIFTs_elements_community[,"S0301"]
# GIFTs_functions_community[,"S03"]
# GIFTs_elements_community[c(1:10),'S0301']
# GIFTs_functions_community[c(1:10),"S03"]
...
genomes_red <- genome_counts_filt %>%
  select(any_of(red_samples)) %>%
  filter(rowSums(across(where(is.numeric)))>0) %>%
  rownames_to_column(var='genome') %>%
  distinct(genome) %>%
  pull()
genomes_grey <- genome_counts_filt %>%
...
diff_fun <- fun_gift_clean %>%
  ...
  summarise(
    ...
    diff = wilcox.test(value ~ species, conf.int = TRUE)$estimate,
    ...
    mean_diff = mean_sc - mean_sv,
    log2fc = log2(mean_sc / mean_sv)) %>%
  mutate(p_adjust=p.adjust(p_value, method="BH")) %>%
  mutate(enrichment = ifelse(diff >0, "Sciurus carolinensis","Sciurus vulgaris"))  %>%
...
diff_fun %>%
  ...
  ggplot(aes(x=forcats::fct_reorder(Function, diff), y=diff, fill=enrichment)) + 
  geom_col() +
  #  geom_point(size=4) + 
  scale_fill_manual(values=squirrel_colors) + 
...
diff_elements %>%
  #  mutate(Difference_abs = abs(Difference)) %>% 
  mutate(element_label=paste(Code_element,Element, sep="-"),
         color = ifelse(p_adjust < 0.05 & (log2fc < -0.6 | log2fc > 0.6), Function, NA),
         sig_label = ifelse(p_adjust < 0.05 & (log2fc < -0.6 | log2fc > 0.6), 
                            Element, NA)) %>% #change Element to element_label to include codes
  ggplot(., aes(x = log2fc, y = -log10(p_adjust), color=color)) + 
  geom_point(size=3)+
  geom_text_repel(aes(label = sig_label), vjust = -0.5, check_overlap = TRUE, min.segment.length=0.4, show.legend=FALSE) +
  scale_color_manual(values = gift_color) +
  theme_classic()+
  labs(size="Mean difference (abs)", color="Functional trait") +
  labs(x = "Log-fold change", y="-Log adjusted p-value") +
