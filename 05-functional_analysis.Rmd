...
function_heatmap_top <- GIFTs_elements %>%
  ...
  geom_tile(aes(y = 1, fill = Function, color = Function), width = 0.9, height = 0.1) +  # Adjust width/height
  geom_text(data = . %>% distinct(Function, .keep_all = TRUE),
            aes(y = 1.08, label = Function), vjust = 0.8, hjust = 0, size = 3, angle = 90) +  # Adjust text placement
  ...
  ylim(0.95, 1.5)  # Adjust y-limits to control label position


mag_elements %>% insert_left(gift_tree, width = .3) %>% insert_top(function_heatmap_top, height=.2)




# # original version (x=MAGs; y=elements)
# 
# mag_elements <- GIFTs_elements %>%
#   as_tibble(., rownames = "MAG") %>%
#   reshape2::melt() %>%
