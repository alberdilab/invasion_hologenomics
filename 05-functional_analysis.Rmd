...
```{r plot1_6, comment="", echo=FALSE, message=FALSE, warning=FALSE, fig.height=8, fig.width=10, eval=FALSE}

# diff_elements %>%
#   #  mutate(Difference_abs = abs(Difference)) %>% 
#   ggplot(., aes(x = mean_diff, y = -log10(p_adjust), color=enrichment)) + 
#   geom_point(size=2, show.legend = FALSE) + 
#   scale_color_manual(values=squirrel_colors)+
#   #xlim(c(-10,4)) +
#   theme_classic()+
#   geom_text_repel(aes(label = Element), size=2)
```
...
diff_elements %>%
  ...
  mutate(color = ifelse(p_adjust < 0.05 & (log2fc < -0.6 | log2fc > 0.6), enrichment, NA),
         sig_label = ifelse(p_adjust < 0.05 & (log2fc < -0.6 | log2fc > 0.6), 
                            Element, NA)) %>%
  ggplot(., aes(x = log2fc, y = -log10(p_adjust), color=color, size=1.5, label=Code_element)) + 
  geom_point()+
...
```{r plot_logfc, comment="", echo=FALSE, message=FALSE, warning=FALSE, fig.height=8, fig.width=10,eval=FALSE}

# diff_elements %>%
#   filter(p_adjust<0.05) %>%
#   ggplot(., aes(x = mean_diff, y = logfc, color=enrichment)) +
#   geom_point(size=2, show.legend = FALSE) + 
#   scale_color_manual(values=squirrel_colors)+
#   #xlim(c(-10,4)) +
#   theme_classic()+
#   geom_text_repel(aes(label = Element), size=2, max.overlaps = Inf)
...
```{r elem_box, comment="", echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
