...
```{r}
#ALPHA DIVERSITY
q0n <- hilldiv(genome_counts,q=0) %>% c()
q1n <- hilldiv(genome_counts,q=1) %>% c()
q1p <- hilldiv(genome_counts,q=1,tree=tree) %>% c()
dist <- traits2dist(genome_kegg_filt, method="gower")
q1f <- hilldiv2::hilldiv(genome_counts_filt,q=1,dist=dist) %>% c()
dist2 <- traits2dist(genome_gifts_filt, method="gower")
...
```{r}
sample_metadata$area_type <-factor(sample_metadata$area_type, levels = c("rural", "semirural", "urban"))
...
```{r}
sample_metadata$season <-factor(sample_metadata$season, levels = c("spring-sum", "autumn", "winter"))
