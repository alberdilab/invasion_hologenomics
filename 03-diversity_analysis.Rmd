...
#functional alpha by species*development
alpha_div %>%
  select(sample,func) %>%
  pivot_longer(-sample, names_to = "data", values_to = "value") %>%
  mutate(data = factor(data, levels = columns))   %>%
...
plot(diversity.data$est_mapp, diversity.data$neutral)  

diversity.data <- diversity.data %>%
  filter(mags_singlem > 0.8) #remove 5 samples with low mapping rate
...
Anova(neutral.model, test.statistic='F', type=3) %>% tt()
...
# plot(effect('builtup_st', neutral.model))
