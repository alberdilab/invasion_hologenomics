...
ehi_phylum_colors2 <- ehi_phylum_colors %>%
  mutate(colors2 = if_else(phylum=="p__Bacillota_C", '#a2088c',
          if_else(phylum=="p__Bacillota_B", '#3fa208',
          if_else(phylum=="p__Bacillota", '#a2083f',
          if_else(phylum=="p__Verrucomicrobiota", '#9d8dad',
          if_else(phylum=="p__Cyanobacteriota", '#08a26b', colors))))))
          
# Arrange colors alphabetically
colors_alphabetic2 <- ehi_phylum_colors2 %>%
  right_join(genome_metadata, by=join_by(phylum == phylum)) %>%
  arrange(match(genome, genome_tree$tip.label)) %>%
...
tSNE_func2$Y %>%
  ...
  scale_color_manual(values=colors_alphabetic2) +
  theme_minimal() +
  theme(legend.position = "right")
```
