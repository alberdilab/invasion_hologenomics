...
```{r hmsc_varpart_red, warning=FALSE, comments="", message=FALSE, cache=TRUE}
...
```{r hmsc_varpart_grey, warning=FALSE, comments="", message=FALSE, cache=TRUE}
...
```{r hmsc_postest_red, warning=FALSE, comments="", message=FALSE, fig.dim=c(8, 10), cache=TRUE}
...
```{r hmsc_postest_grey, warning=FALSE, comments="", message=FALSE, fig.dim=c(8, 10), cache=TRUE}
...
Log-abundance differences between autumn and spring-summer in Sciurus vulgaris for genera found enriched in either season.

```{r season_plot_red, comment="", message=FALSE, warning=FALSE, fig.dim=c(8,10), fig.fullwidth=TRUE}
...
pred_season_red %>%
  ...
  left_join(aut_enrichment_red,by="genome") %>%
      ...
      ggplot(., aes(y=forcats::fct_reorder(genus,diff_sa), x=diff_sa, fill=phylum, color=phylum)) +
            ...
            annotate('text', x=3, y=18, label = "Enriched in\nautumn", color='black') +
...
pred_season_red %>%
    ...
    left_join(win_enrichment_red,by="genome") %>%
      ...
      ggplot(., aes(y=forcats::fct_reorder(genus,diff_sw), x=diff_sw, fill=phylum, color=phylum)) +
            ...
            scale_color_manual(values=custom_colors)+
            scale_fill_manual(values=alpha(custom_colors,0.4))+
            annotate('text', x=10, y=18, label = "Enriched in\nwinter", color='black') +
            annotate('text', x=-10, y=18, label = "Enriched in\nspring-summer", color='black') +
            geom_vline(xintercept = 0)+
            theme_classic()+
            labs(x = "Difference in log-abundance", y = "Genera") +
...
```{r hmsc_season_enrichment_grey, warning=FALSE, comments="", message=FALSE, fig.dim=c(8, 10)}
...
pred_season_grey %>%
  ...
  left_join(aut_enrichment_grey,by="genome") %>%
      ...
      ggplot(., aes(y=forcats::fct_reorder(genus,diff_sa), x=diff_sa, fill=phylum, color=phylum)) +
            ...
            scale_fill_manual(values=alpha(custom_colors,0.4))+
            geom_vline(xintercept = 0)+
            annotate('text', x=2.5, y=15, label = "Enriched in\nautumn", color='black') +
            annotate('text', x=-2.5, y=15, label = "Enriched in\nspring-summer", color='black') +
            theme_classic()+
            labs(x = "Difference in log-abundance", y = "Genera") +
            xlim(-5,5)
...
pred_season_grey %>%
  ...
  left_join(aut_enrichment_grey,by="genome") %>%
      ...
      ggplot(., aes(y=forcats::fct_reorder(genus,diff_sw), x=diff_sw, fill=phylum, color=phylum)) +
            ...
            scale_fill_manual(values=alpha(custom_colors,0.4)) +
            geom_vline(xintercept = 0) +
            annotate('text', x=5, y=15, label = "Enriched in\nwinter", color='black') +
            annotate('text', x=-5, y=15, label = "Enriched in\nspring-summer", color='black') +
            theme_classic()+
            labs(x = "Difference in log-abundance", y = "Genera") +
            xlim(-10,10)
...
present_MAGs <- genome_counts %>%
  filter(rownames(.) %in% m.red$spNames) %>%
  rownames()
...
```{r function_urb_tables_red, message=FALSE, warning=FALSE}
...
all_functions_red <- function_pred_urb_red %>%
...
```{r function_urb_tables_grey, message=FALSE, warning=FALSE}
...
all_functions_grey <- function_pred_urb_grey %>%
  left_join(GIFT_db,by=join_by(trait==Code_function)) %>%
  mutate(trait=factor(trait)) %>%
  mutate(function_legend=str_c(trait," - ",Function)) %>%
  select(trait,mean,p1,p9,function_legend) %>% 
...
all_functions_red %>%
  ggplot(aes(x=mean, y=fct_reorder(function_legend, mean), xmin=p1, xmax=p9, color=function_legend)) +
      ...
      scale_color_manual(values = gift_colors$Color) +
      theme_minimal() +
      labs(x="Spring-winter",y="Functional trait") +
      #guides(col = guide_legend(ncol = 1))
      theme(legend.position = "none")
